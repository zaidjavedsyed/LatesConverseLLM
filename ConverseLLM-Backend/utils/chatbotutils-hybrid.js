...
async function loadWithFallback(url) {
    ...
    console.log('🔄 Using hybrid loader for:', url);
    
    try {
        // Step 1: Try RecursiveUrlLoader first (faster)
        console.log('   📋 Step 1: Trying RecursiveUrlLoader...');
        ...
        const recursiveDocs = await recursiveLoader.load();
        console.log(`   ✅ RecursiveUrlLoader loaded ${recursiveDocs.length} pages`);
        
        // Check if content is sufficient
        const hasGoodContent = recursiveDocs.length >= 2 && 
                              recursiveDocs.some(doc => doc.pageContent.length > 200);
        
        if (hasGoodContent) {
            console.log('   ✅ Content sufficient, using RecursiveUrlLoader results');
            return recursiveDocs;
        }
        
        console.log('   ⚠️ Content too small, falling back to Playwright...');
        
    } catch (err) {
        console.log('   ❌ RecursiveUrlLoader failed:', err.message);
        console.log('   🔄 Falling back to Playwright...');
    }
    
    // Step 2: Fallback to Playwright (more thorough)
    try {
        console.log('   📋 Step 2: Using Playwright loader...');
        const { PlaywrightWebBaseLoader } = require('langchain/document_loaders/web/playwright');
        
        const playwrightLoader = new PlaywrightWebBaseLoader(url, {
            launchOptions: { 
                headless: true,
        ...
        console.log(`   ✅ Playwright loaded ${playwrightDocs.length} pages`);
        return playwrightDocs;
        
    } catch (err) {
        console.log('   ❌ Playwright also failed:', err.message);
        throw new Error(`Both loaders failed. RecursiveUrlLoader: ${err.message}`);
    }
}
//const { HierarchicalNSW } = require("hnswlib-node");
   ...
   // console.log('In createChatBot func');
    ...
    if(fileExists){
      ...
      //  console.log("Creating Vector Store..");
        ...
        // Use hybrid loader with intelligent fallback
        const vecdocs = await loadWithFallback(url);
